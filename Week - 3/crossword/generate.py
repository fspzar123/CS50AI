...
class CrosswordCreator():
        ...
        if overlap:
            ...
            for x_word in self.domains[x]:
                if not any(
                    ...
                    x_to_remove.add(x_word)
            if x_to_remove:
                self.domains[x] = self.domains[x].difference(x_to_remove)
                revised = True
        return revised
    ...
    def ac3(self, arcs=None):
        ...
        """
        if arcs is None:
            arcs = [(var, neighbor) for var in self.crossword.variables for neighbor in self.crossword.neighbors(var)]
        else:
            arcs = list(arcs)
    ...
    def order_domain_values(self, var, assignment):
        ...
        return sorted(
            ...
            key=lambda value: sum(
                ...
                if self.crossword.overlaps[var, neighbor] and value[self.crossword.overlaps[var, neighbor][0]] != neighbor_value[self.crossword.overlaps[var, neighbor][1]]
            )
        )
